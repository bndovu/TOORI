...
const CharacterGenerator: React.FC<CharacterGeneratorProps> = ({
    ...
    const [selectedStyle, setSelectedStyle] = useState<ImageStyle>('Vibrant');
                        ...
                        {char.imageUrl ? (
                            ...
                            <div className="w-full aspect-square bg-black/20 rounded-md flex items-center justify-center">
                             ...
                             <div className="flex flex-col space-y-2">
                                <div className="flex items-center space-x-2 rounded-lg bg-black/20 p-1">
                                    {imageStyles.map(style => (
                                        <button
                                            ...
                                            className={`w-full py-1 text-xs font-semibold rounded-md transition-colors duration-300 focus:outline-none focus:ring-1 focus:ring-[var(--color-focus-ring)] ${selectedStyle === style ? 'bg-white/20' : 'text-[var(--color-text-tertiary)] hover:bg-white/10'}`}
                                        >
                                            {style}
                                        </button>
                                    ))}
                                ...
                                <button
                                    onClick={() => onGenerateImageForCharacter(index, selectedStyle)}
